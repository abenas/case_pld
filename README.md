...
1.  Clonar o projeto do github.
2.  Subir a infraestrutura do KAFKA.
3.  Subir a infraestrutura ELK.
4.  Subir as aplicações, LOADER, MODERN-PRODUCER-A e MODERN-PRODUCER-B.
5.  Submeter uma requisição POST em `localhost:8081/transactions` (utlizar a collection postman data/pld-postman-collection.json).
6.  Submeter um evento via KafkaAdmin em `localhost:9092` tópico *bridge* (utilizar o scripts/help.txt).
...
10. Dado que é um projeto importantíssimo para a organização, como você faria para acelerar o desenvolvimento, entregando valor para o cliente em menor tempo?
    ...
    Utilizando tecnologias consolidadas e amplamente utilizadas no mercado, com cases provados e templates para os cenários possíveis.
